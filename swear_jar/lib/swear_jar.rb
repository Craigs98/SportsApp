  ...
  module ClassMethods
    ...
    def swear_jar(*attr_names)
      ...
      attr_names.each do |attr_name|
        instance_eval do
          define_method "#{attr_name}_clean" do; SwearJar::Base.clean(self[attr_name.to_sym], option); end
    ...
    def setup_callbacks_for(attr_name, option)
      before_validation do |record|
        record[attr_name.to_sym] = SwearJar::Base.clean(record[attr_name.to_sym], option)
      end
    end
  end
...
ActiveRecord::Base.send(:include, SwearJar) if defined?(ActiveRecord)
